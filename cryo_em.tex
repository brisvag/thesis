...
This method comes at the cost of worse SNR due to the thicker sample, and cannot provide full orientation coverage due to the limited tilt angle achievable, so the aforementioned approaches are generally preferred.
...
This is a useful feature for most applications, because we often care about where exactly the similarity is high (e.g. for motion correction and particle picking (\fullref{em_particle_picking})).
...
To improve the picking speed and precision, template matching is often used to automate searching for a recognizable object (\autoref{fig:em_template}).
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=.5\textwidth]{example-image.png}
    \caption{TODO: templates and picks on micrograph}
    \label{fig:em_template}
\end{figure}
...
Even with the best data and most advanced methods, picked particles typically contain a variety of several different objects, conformations, and spurious background picks; before moving on to reconstruction, the wheat should be separated from the chaff.
...
Once a 3D model is obtained, several state of the art software pipelines offer the ability to refine previously-defined parameters, using the estimated resolution of the 3D reconstruction to validate the improvements.

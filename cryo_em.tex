...
To mitigate the effect of loss of coherence, inelastically scattered electrons are filtered out using an energy filter, which removes electrons with energy that differs significantly from the unscattered ones (\autoref{fig:em_image_formation}).
...
For this reason, a certain amount of defocus is always used, which "compresses" the CTF thus improving the low frequency contrast transfer; this comes unfortunately at the cost of jumbling up the high frequency information with more CTF oscillations, making it harder to correctly estimate the CTF at high resolutions (\autoref{fig:em_ctf_defocus_1d}).

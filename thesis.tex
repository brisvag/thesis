...
\usepackage{dirtytalk} % simple quotes (\say{something})
%\usepackage{tikz}
...
\usepackage{csquotes} % normal-er quotes (import after minted to fix warning)
% \setminted[python]{linenos} % enable line numbers on all python boxes
\usepackage{mdframed} % framing minted boxes
\definecolor{bg}{rgb}{0.95,0.9,0.75}
\surroundwithmdframed[backgroundcolor=bg]{minted}
...
\usepackage[blocks]{authblk} % multiple authors with affiliations (must import after titling)
\renewcommand\Affilfont{\scriptsize}
\usepackage{pdfpages} % include pre-rendered pdfs
% switch out next 3 lines with hidelinks line for printer-friendly
% renewcommand is for back-referencing the text from the bibliography section
...
\usepackage{minitoc} % per-chapter TOC. Must be after hyperref
\mtcsetrules{*}{off}
\dominitoc
\usepackage{lipsum}
